"""
Script to synchronize data folders across workspace root and bar_inventory_project,
update `build_notebook.py` path logic, and re-execute the Jupyter notebook cleanly.
"""
import os
import shutil
...
# 1. Sync data files so both relative paths work seamlessly
os.makedirs("data/raw", exist_ok=True)
os.makedirs("data/processed", exist_ok=True)
os.makedirs("bar_inventory_project/data/raw", exist_ok=True)
os.makedirs("bar_inventory_project/data/processed", exist_ok=True)

if os.path.exists("bar_inventory_project/data/raw/bar_inventory_data.csv"):
    shutil.copy("bar_inventory_project/data/raw/bar_inventory_data.csv", "data/raw/bar_inventory_data.csv")

if os.path.exists("bar_inventory_project/data/processed/daily_bar_consumption.csv"):
    shutil.copy("bar_inventory_project/data/processed/daily_bar_consumption.csv", "data/processed/daily_bar_consumption.csv")

# 2. Re-create notebook with multi-candidate path resolution
...
"""))
    cells.append(new_code_cell("""raw_candidates = [
...
"""))
    ...
    # Quality Checks & Inventory Conservation Validation
...
"""))
    cells.append(new_code_cell("""daily_ts = daily_ts.sort_values(['Bar Name', 'Brand Name', 'Date']).reset_index(drop=True)
...
"""))
    ...
    # Dynamic Par Level & Safety Stock Calculation
...
"""))
    ...
    # Answers to 5 Business Questions
...
nb_obj = create_bulletproof_notebook()

nb_locations = [
    "bar_inventory_project/notebooks/inventory_forecasting_solution.ipynb",
...
print("Notebook generated in target locations.")

# Execute notebook in-place
target_nb_path = "bar_inventory_project/notebooks/inventory_forecasting_solution.ipynb"
print(f"Executing {target_nb_path} in-place...")
...
# Also sync to notebooks/ folder if needed
if os.path.exists("notebooks/inventory_forecasting_solution.ipynb"):
    shutil.copy(target_nb_path, "notebooks/inventory_forecasting_solution.ipynb")

print("SUCCESS: Notebook executed and saved without errors!")
